<template>
    <layout title="Articulos">
        <h1 class="mb-8 font-bold text-3xl">Articulos</h1>
        <div class="mb-6 flex justify-between items-center">
            ...
            <inertia-link class="btn-yellow" :class="{'hidden':articles.data.length === 0}" :href="route('inventory.articles.create', organization.slug)">
                <span>Crear</span>
                <span class="hidden md:inline">Articulo</span>
            </inertia-link>
        </div>
        <div class="bg-white rounded shadow overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                <tr class="text-left font-bold">
                    ...
                    <th class="px-6 pt-6 pb-4">Costo</th>
                    <th class="px-6 pt-6 pb-4" colspan="2">Descripción</th>
                </tr>
                <tr v-for="article in articles.data" :key="article.id" class="hover:bg-grey-lightest focus-within:bg-grey-lightest">
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center focus:text-green" :href="route('inventory.articles.edit', {'slug':organization.slug, 'article':article.id})">
                            {{ article.name }}
                            <icon v-if="article.deleted_at" name="trash" class="flex-no-shrink w-3 h-3 fill-grey ml-2" />
                        </inertia-link>
                    </td>
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center" :href="route('inventory.articles.edit', {'slug':organization.slug, 'article':article.id})" tabindex="-1">
                            {{ article.cost }}
                        </inertia-link>
                    </td>
                    <td class="border-t">
        ...
        watch: {
            form: {
                handler: _.throttle(function() {
                    ...
                    this.$inertia.replace(this.route('inventory.articles',  {
