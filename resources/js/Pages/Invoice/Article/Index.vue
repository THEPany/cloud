<template>
    <layout title="Articulos">
        ...
        <div class="mb-6 flex justify-between items-center">
            ...
            <inertia-link class="btn-green" :class="{'hidden':articles.data.length === 0}" :href="route('invoice.articles.create', organization.slug)">
                <span>Crear</span>
        ...
        <div class="bg-white rounded shadow overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tr v-for="article in articles.data" :key="article.id" class="hover:bg-grey-lightest focus-within:bg-grey-lightest">
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center focus:text-green" :href="route('invoice.articles.edit', {'slug':organization.slug, 'article':article.id})">
                            {{ article.name }}
                            <icon v-if="article.deleted_at" name="trash" class="flex-no-shrink w-3 h-3 fill-grey ml-2" />
                        </inertia-link>
                    </td>
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center" :href="route('invoice.articles.edit', {'slug':organization.slug, 'article':article.id})" tabindex="-1">
                            {{ article.cost }}
                        </inertia-link>
                    </td>
                    <td class="border-t">
                    ...
                    <td class="border-t w-px">
                        <inertia-link class="px-4 flex items-center" :href="route('invoice.articles.edit', {'slug':organization.slug, 'article':article.id})" tabindex="-1">
                ...
                <tr v-if="articles.data.length === 0" class="">
                    <td class="border-t px-6 py-4" colspan="3">
                        <div class="text-center flex flex-wrap justify-center">
                            ...
                            <inertia-link class="btn-green" :href="route('invoice.articles.create', organization.slug)">
                                <span>Crear</span>
                                <span class="hidden md:inline">Articulo</span>
                            </inertia-link>
                        </div>
        ...
        watch: {
            form: {
                handler: _.throttle(function() {
                    ...
                    this.$inertia.replace(this.route('invoice.articles.index',  {
