    ...
    <layout title="Articulos">
        ...
        <div class="mb-6 flex justify-between items-center">
            <search-filter v-model="form.search" class="w-full max-w-sm mr-4" @reset="reset">
                <label class="block text-grey-darkest">Basura:</label>
            ...
            <inertia-link class="btn-green" :class="{'hidden':payments.data.length === 0}" :href="route('invoice.payments.create', organization.slug)">
                <span>Crear</span>
                <span class="hidden md:inline">Pago</span>
            </inertia-link>
        </div>
        <div class="bg-white rounded shadow overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                <tr class="text-left font-bold">
                    <th class="px-6 pt-6 pb-4">No. Pago</th>
                    <th class="px-6 pt-6 pb-4">Fecha Pago</th>
                    <th class="px-6 pt-6 pb-4" colspan="2">Cantidad recibida</th>
                </tr>
                <tr v-for="payment in payments.data" :key="payment.id" class="hover:bg-grey-lightest focus-within:bg-grey-lightest">
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center focus:text-green" :href="route('invoice.payments.show', {'slug':organization.slug, 'payment':payment.id})">
                            {{ payment.id }}
                            <icon v-if="payment.deleted_at" name="trash" class="flex-no-shrink w-3 h-3 fill-grey ml-2" />
                        </inertia-link>
                    </td>
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center" :href="route('invoice.payments.show', {'slug':organization.slug, 'payment':payment.id})" tabindex="-1">
                            {{ payment.created_at }}
                        </inertia-link>
                    </td>
                    <td class="border-t">
    ...
    watch: {
      form: {
        handler: _.throttle(function() {
          ...
          this.$inertia.replace(this.route('invoice.payments.index',  {
