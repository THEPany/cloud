<template>
    <layout title="Home">
        <h1 class="mb-8 font-bold text-3xl">Colaboradores</h1>
        <div class="bg-white rounded shadow overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tr v-for="user in contributors.data" :key="user.id" class="hover:bg-grey-lightest focus-within:bg-grey-lightest">
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center focus:text-indigo" :href="route('setting.permissions.show', {'slug':organization.slug, 'user':user.id})">
                            {{ user.name }}
                        </inertia-link>
                    </td>
                    <td class="border-t">
                        <inertia-link class="px-6 py-4 flex items-center" :href="route('setting.permissions.show', {'slug':organization.slug, 'user':user.id})" tabindex="-1">
                            {{ user.email }}
                        </inertia-link>
                    </td>
                    <td class="border-t w-px">
                        <inertia-link class="px-4 flex items-center" :href="route('setting.permissions.show', {'slug':organization.slug, 'user':user.id})" tabindex="-1">
